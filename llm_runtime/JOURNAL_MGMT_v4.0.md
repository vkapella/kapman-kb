...
One file per ticket under `log/tickets/<YYYY-MM>/`, written by Pass 2 for every Validated recommendation — never for Flagged or Rejected. The ticket id extends the run spine: `<run_id>/P2-NN/T1`, filename-safe as `<run_id>_P2-NN_T1.md`. A ticket is a **proposal record, not a position**: writing one creates no `positions.md` entry, triggers no entry-time snapshot, and asserts nothing about execution — it records what Pass 2 authorized, so any later fill can be measured against the specification that governed it.

The PROPOSED ticket carries: the **instrument block** — root, expiration, right, strike, and the derived `osi_symbol` (the exact broker-format contract string, computed, never typed) — or, for a multi-leg structure, the **legs list** per the spread-ticket clause below; the Pass-2 fields — structure, entry range, chain quality, sizing band; the **exit contract** — stop anchor {level, basis, dealer window, durability {touch_prob, atr_multiple}}, targets {level, probability, horizon}, trail {unit, per-venue basis}, earnings/catalyst advisories; the **manifest** — KB commit, model, `screen_version`, `v2_schema_version`, denominator {value, source, as-of, status} when declared and named absent otherwise; the **account** (entity-scoped destination when known, else `unassigned`; on an entity-stamped run the destination must belong to the run's `legal_entity`, and `unassigned` is not approvable — no APPROVED event may be written against it); `side: open` (`close` is reserved vocabulary for a later increment — never written by this rule); `status: PROPOSED`.
...
**A spread ticket carries a legs list; everything else about it is a single ticket.** When Pass 2 validates a multi-leg structure (`CALL_DEBIT_SPREAD` / `PUT_DEBIT_SPREAD`), the ticket's instrument block becomes `legs[]` — per leg: the full instrument block (root, expiration, right, strike, derived `osi_symbol`), `position: long | short` (a per-leg axis, deliberately not `side`, which remains the ticket-level open/close verb), and `ratio` (integer; 1 for verticals — carried so a future non-1:1 structure is an extension, not a migration). The entry range covers the **net debit** — the spread is priced as one object, and the individual leg prices are not promised separately, exactly as the Pass 2 entry-range clause already states. Chain quality, sizing band, the exit contract, the manifest, and the account are position-level, unchanged. One recommendation is still one ticket (`…/P2-NN/T1`); lifecycle, TTL, expiry, and event records are untouched. At approval, `limit_price` is the net debit and must fall inside the net-debit entry range; `quantity` is the number of spreads — the per-leg contract count is `quantity × ratio`, derived never typed.

**Lifecycle is a chain of event records, never an edit.** Each transition is its own file beside the ticket — `_approved.md`, `_rejected.md`, `_expired.md`, `_executed.md` — so every state change carries its own timestamp and author, and no ticket file is ever reopened; append-only survives multiple writers by construction. The APPROVED event is where broker-facing values are born: {limit_price — inside the ticket's entry range or carrying an explicit override note; duration; instruction — derived from structure + side, stamped never typed; quantity — computed from the sizing band against the run's entity-scoped denominator at the approved limit (the declared-entity denominator per `RISK_v4.0.md`); account}. The EXECUTED event records the match to a broker fill via the §A2 import join — written when the fill is confirmed, not when an order is sent. Writing it has one consequence beyond the event file itself: the entry-time snapshot captured on the parent Pass 2 record (`parent_pass2` via the ticket's lineage) is transcribed verbatim into `memory/positions.md`, keyed `(instrument_key, account_id)` — the one moment a validated specification becomes a position. Transcription copies; it never re-fetches, recomputes, or updates the captured values.
...
**The run's recommendation rows are also POSTed to the tradelog measurement plane — a mirror, never a second authority.** Alongside the end-of-run journal commit, the session POSTs the run's Pass 1 and Pass 2 rows to tradelog `POST /api/recommendations` — idempotent, keyed on `rec_id`, so a re-POST never double-logs. The journal remains the record of authority; the tradelog copy exists for measurement (plan-vs-actual) and display, and is rebuildable from the journal at any time. The POST rides the same failure discipline as every journal write: a failed or skipped POST degrades to a reported-on-screen error and never blocks the report or the commit. The Rule 7 manifest names the POST outcome — rows sent, created, updated, or the named failure.
...
| Pass 1 records (output) | `log/pass1/<YYYY-MM>/<run_id>.md` |
| Pass 2 records (output) | `log/pass2/<YYYY-MM>/<run_id>.md` |
| Trade tickets (output) | `log/tickets/<YYYY-MM>/<run_id>_P2-NN_T1.md` |
| Ticket lifecycle events | `log/tickets/<YYYY-MM>/<run_id>_P2-NN_T1_<event>.md` |
...
- **Ticket ids** extend the run spine: `<run_id>/P2-NN/T1` (record id) ↔ `<run_id>_P2-NN_T1` (filename form); event files append `_approved` / `_rejected` / `_expired` / `_executed`.

**Run ID format.** `<lineage_id>-RNN` — per-source consumer sequence, `-R1` first.
- Run-record filenames use the run_id stem (`log/pass1/<YYYY-MM>/<run_id>.md`); handoff filenames stay `<lineage_id>.md`.
- Ticket ids extend the run spine, so rec_ids from two runs off one handoff cannot collide.
- Frontmatter carries `run_id`, `source_lineage_id` (verbatim, never reformatted), `legal_entity`, `environment`.
- A record with no run suffix is a legacy single-run record whose run *is* its lineage; records lacking scope frontmatter are **LEGACY_UNSCOPED**.
...
environment: live                    # portfolio_snapshot only — echoed from scope.environment
```

**Pass 1 record header** (per §A4): `rec_id`, `run_id`, `source_handoff: <lineage_id>`, `legal_entity`, `environment`, `{date, ticker, structure, pass}`, disposition (Eligible | NO_TRADE | WAIT) + reason, candidate zone, `decided_at`, `underlying_ref`, `journal_schema_version`, reserved `attack_flags[]` / `invalidation_conditions[]` (empty until Stage 3) — and, when the source envelope carried it, `feed_view {id, name}` (the pipeline-feed preset that filtered the cohort; absent means "feed not recorded", never inferred).

**Pass 2 record header** (per §A4): `rec_id`, `run_id`, `parent_pass1: <rec_id>`, `legal_entity`, `environment`, `{ticker, strike, expiration}`, exact spec, entry range, targets, `option_mid`, `decided_at`, `journal_schema_version` — and, for Validated recommendations, the **entry-time snapshot block**: the five regime fields, the eight SIGNAL stop/profit levels, and the entry-context riders, held here immutably until (and unless) a ticket EXECUTED event transcribes them into `positions.md`. A never-executed validation keeps its snapshot in the log forever and it goes nowhere — a record of conditions, not a position.
