...
|---|---|---|
| IV/HV band (cheap / neutral / elevated) — per ticker | `SIGNAL_v3.0.md` | Triggers the spread-mandate decision; elevated band → spread required; neutral → spread preferred; cheap → naked long-premium eligible |
...
| IV rank tier (low / moderate / elevated / extreme) — per ticker | `RISK_v3.0.md`, `PASS1_SCREENING_v3.0.md`, `PASS2_VALIDATION_v3.0.md` | RISK reads tier as a sizing-band qualifier; PASS1 may filter candidates by tier; PASS2 reads tier into structure-validation context |
...
| IV/HV source-authority (one producer, both passes) | `PASS1_SCREENING_v3.0.md`, `PASS2_VALIDATION_v3.0.md`, `engineering_only/VOLATILITY_MCP_REFERENCE_v3.0.md` | Both passes call the Polygon options-metrics producer; PASS2 re-confirms on a fresh fetch. MCP reference holds the tool-surface names and parameter shapes |
...
- `SIGNAL_v3.0.md` — consumes the IV/HV band as the trigger for the spread-mandate decision. The named trigger formerly known as `iv_forbids_long_premium` is operationalized as "IV/HV band reads elevated"; SIGNAL owns the trigger's behavioral consequence (spread required for new directional entries), VOLATILITY owns the band definition.
- `PASS1_SCREENING_v3.0.md` — consumes IV rank tier as a candidate filter input and consumes the volatility-status label for confidence weighting. PASS1 reads the IV/HV band from the Polygon options-metrics producer (`atm_iv` ÷ HV20) per the source-authority rule.
- `PASS2_VALIDATION_v3.0.md` — consumes the IV/HV band for structure validation and re-confirms the mandate by re-fetching the Polygon options-metrics producer against a fresh chain. PASS2's chain-quality categorization (full / limited / weak) and VOLATILITY's volatility-status label (FULL / LIMITED / INVALID) are distinct dimensions and both must be honored.
...
The 1.20 threshold for the elevated band is the v2.3 carryover referenced in `RISK_v3.0.md` ("elevated IV (≥ 1.2)") and is preserved here as the v3.0 reference value. The neutral band (roughly 0.96 to 1.19) is new content in v3.0: v2.3 had a single threshold separating "spread mandatory" from "naked OK," and v3.0 introduces the neutral band to give RISK sizing room to breathe at near-1.0 ratios where neither structure is clearly forced. The cheap band ceiling (0.95) is symmetric with the neutral floor. The `IV_HV_ELEVATED_THRESHOLD` (≥ 1.20) and `IV_RANK_EXTREME_FLOOR` (75) values are defined as named parameters in `SYSTEM_PARAMS_v3.0.md`; update that file when recalibrating either threshold.
...
The producer's `iv_hv_status` and `avg_iv_status` are mechanical (the ratio is computable or it is not); the contract-count and history thresholds behind them are MCP-internal parameters documented in `engineering_only/VOLATILITY_MCP_REFERENCE_v3.0.md` (forthcoming).
