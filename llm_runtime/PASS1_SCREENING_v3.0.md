...
| 3 — Regime reads | MCP fetch: initial candidate-list metric fetch may use `get_metrics_batch` (up to 30 symbols per call; caller chunks larger lists); per-ticker `get_wyckoff_proposal_context` is still called individually before trigger evaluation for each candidate. Dealer metrics, volatility metrics (Pass 1 IV source). | Per candidate | Dealer-timing veto; spread-mandate |
