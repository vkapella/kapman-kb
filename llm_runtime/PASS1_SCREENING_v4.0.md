...
The envelope also carries two run-level fields the rows do not. **`pt_horizon_bars`** is the calibration window for every `*_prob` in the export — the number of trading bars the producer's hit rates were measured over — and SIGNAL's horizon-affordability test reads it directly. It is bound at source to the field name the probabilities are parsed through, so the delivered value cannot drift from the window actually measured. **`null` means the catalog was unavailable at export time, not that the horizon is 60**: on null the dependent test degrades to unresolvable and says so, and the runtime never substitutes a default, because a substituted constant that looks like delivered data is worse than an acknowledged gap. **`breadth_context`** is a run-level market-breadth block (`log_date`, `pct_above_sma200`, `delta_above_sma200`, `band`, `symbols`) whose `band` — `strong` / `mixed` / `weak` — is computed by the producer against fixed 35/65 edges. It is **labeled context only and never a gate**: it does not enter the macro gate, does not modify a disposition, and does not affect sizing. The runtime carries the producer's `band` verbatim and **does not re-derive the edges**; `breadth_context` is `null` when the producer has no breadth series yet, which is a data-absent note, not a neutral reading.
...
**Every unresolved pipeline-flagged reading leaves the run as a queue item — the flag survives the session that raised it.**

At the end of a Pass 1 run, each ticker whose Wyckoff reading ends the session `pipeline-flagged` with no in-session resolution is posted to the Tradelog Today queue as one `WYCKOFF_FLAGGED` queue item per `engineering_only/HITL_QUEUE_CONTRACT_v4.0.md`: `queue_item_id = <rec_id>/Q1`, the proposal snapshot carrying exactly what the run showed the operator — the prompt, the decision inputs with their present/null/absent states preserved, the gate result and named flag reasons — and `proposal_hash` computed per the contract. Posting is transport, not judgment: it asserts nothing about the reading and creates no obligation; it keeps the question askable after the session ends, which is exactly what parked the 71-row spring cohort. An in-session resolution produces no queue item — the queue is for questions the session could not finish. The POST rides the same failure discipline as every mirror write: a failed or skipped POST degrades to a reported error and never blocks the report, and the Rule 7 manifest names the queue-POST outcome.
...
**Spring-review fast-path (flagged-reading exchange short form).** When a flagged reading is spring-cohort — an accumulation-family regime (`accumulation` or `reaccumulation`) with a confirmed fresh bullish phase-C decisive test (`spring` or `shakeout`) — and the **only** reason the reading is flagged is `weekly_agrees = false`, the flagged-reading exchange runs a structured short-form resolution instead of the full exchange. The short-form card presents: (1) the spring event date and its bar location relative to range support; (2) the weekly trend read, with the explicit note that a fresh spring *expects* weekly disagreement — a daily accumulation base inside a weekly downtrend is constitutive of the setup, not an anomaly; (3) dealer/veto status from the current reading; (4) the invalidation level. The operator confirms or refuses on that card alone. Any other flag reason — `structure_conflict`, stale snapshot, estimation path, missing fields, or multiple concurrent flag reasons — routes to the full exchange as before. The fast-path is long-side only: the bearish UTAD mirror keeps the full exchange until option-space economics exist for the bear side. Unattended runs are unchanged: an unresolved spring stays WAIT, never auto-accepts, and a vetoed spring is still refused. Standing caution, printed on every short-form card: *tier-gate recalibration for springs (the τ paradox proper) is intentionally frozen until the September 2026 re-evaluation — this fast-path speeds operator resolution; it does not license threshold tuning.* Provenance: the pinned 2026-07-02 economics study (spring+flip the only CI-positive call cohort on the honest universe) and the 2026-07-23 live forward-log read (markup_continuation realized 28.9% hit-base vs 69.5% expected, n=38 — the worst cell in the production scorecard — while the first spring-class validations arrived at day 27 behind auto-accepted continuation names).
