...
When a candidate is validated, Pass 2 captures the entry-time record onto the Pass 2 log record itself — the entry-time regime snapshot (entry Wyckoff regime, DGPI tier, flip-zone, IV/HV band, vol-status), the eight SIGNAL Stop/Profit alert levels, and `option_mid` — the validated-chain bid/ask midpoint at entry — plus the best-effort entry-context riders (entry phase A–E, `phase_c_confirmed`, and the confirmed `entry_wyckoff_event`). The values are validation-time by definition, and the ticket's expiry window is what keeps them honest as entry context: a fill can occur no later than the first 30 minutes of the next regular session, so the captured conditions and the actual entry are never more than one session apart. **Validation writes no `positions.md` entry.** A validated specification is not a position, and it is not a stored exit contract — sixteen unexecuted validations written as open positions would corrupt the real position ledger, which is exactly what live practice refused on 2026-08-07 and what the fork-reconciliation record upheld. The snapshot becomes a `positions.md` record at exactly one moment: the ticket's EXECUTED event, when a broker fill is confirmed via the §A2 import join and `JOURNAL_MGMT`'s transcription clause copies the captured block verbatim. The regime snapshot's five fields and the eight SIGNAL levels remain the one regime read Pass 2 persists: the sole exemption to the numeric-no-persist floor, held as immutable historical entry context — a record, not an authority — never re-read to seed a Pass 1 or Pass 2 decision, since a fresh decision always re-fetches the live regime. `option_mid` rides alongside as a position fact, and the riders as non-exempt categorical entry context — both outside the exemption proper. Pass 2 owns the field capture and its timing — validation; `JOURNAL_MGMT_v4.0` owns the transcription trigger, path, schema, and write-once mechanics (including the riders' grammar), and `KAPMAN_GUARDRAILS` owns the exemption. A position entered outside the ticket flow — a manual buy with no Pass 2 behind it, or an executed Flagged candidate, which gets no ticket — has no captured snapshot: it enters position context on a Portfolio refresh with entry context absent, and PORTFOLIO_MGMT's current-session reconstruction and absent-context degrade are the documented path. Reconstruction is always labeled as such; a reconstructed anchor is never backfilled as if it had been captured. The capture changes neither the Pass 1 → Pass 2 boundary nor the anti-hallucination floor: Pass 2 still validates from the live chain and persists a record, not an authority.
...
- `JOURNAL_MGMT_v4.0.md` owns how the entry-time snapshot Pass 2 captures at validation reaches `positions.md` — the transcription trigger (the ticket's EXECUTED event), path, schema, and write-once mechanics. PASS2 owns only the capture (at validation) and the captured field set; it does not define the journal's write model.
