...
This file documents the portfolio-management MCP and persistence mechanics named by `llm_runtime/PORTFOLIO_MGMT_v4.0.md`. The runtime PORTFOLIO_MGMT file owns position lifecycle behavior, regime monitoring, advisory state, and portfolio-mode workflow; this file owns endpoint names, parameter shapes, and position-persistence implementation details once those mechanics are specified.
...
| Position context schema | Runtime-owned schema is documented in `llm_runtime/PORTFOLIO_MGMT_v4.0.md` Appendix; no backend persistence implementation is provided in the source anchors. |

### Extracted operational parameter cross-reference

| Parameter | Extracted value |
|---|---|
| `DTE_DECAY_WARNING_THRESHOLD` | Consumed from `SYSTEM_PARAMS_v4.0.md`; no numeric value is owned by this file. |

## Legacy anchors

No legacy rule IDs map to this file. `PORTFOLIO_MGMT_v4.0.md` is a net-new v3.0 construct with no v2.3 antecedent.
